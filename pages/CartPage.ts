...
export class CartPage {
  ...
  async verifyProductInCart(productName: string) {
    ...
    const productLocator = this.page.locator(this.cartItemDescription, {
      hasText: productName,
    });
  ...
  async verifyProductQuantity(expectedQty: string) {
    const quantityBtn = this.page.locator(".cart_quantity button").first();
    await expect(quantityBtn).toHaveText(expectedQty);
  }

  // Bấm nút xóa hình dấu X màu đỏ ở dòng đầu tiên
  async clickRemoveFirstProduct() {
  ...
  async verifyCartIsEmpty() {
    ...
    if (!hasEmptyCartMessage) {
      ...
      await expect(
        rows,
        "Lỗi: Giỏ hàng không rỗng! Chưa đăng nhập nhưng vẫn thêm được sản phẩm vào giỏ.",
      ).toHaveCount(0);
    } else {
  ...
  async verifyCartItemCount(expectedCount: number) {
    ...
    await expect(
      rows,
      `Lỗi: Giỏ hàng phải có ${expectedCount} sản phẩm.`,
